...
PIXEL_LABELS = [chr(ord("1") + i) for i in range(12)]
...
def find_numato():
    ...
    preferred_ports = [
        ...
        or "numato" in " ".join(
            str(v) for v in (p.manufacturer, p.description, p.product)
        ).lower()
    ]
...
def numato_relay_token(ch):
    if not isinstance(ch, int) or not 0 <= ch < NUMATO_RELAY_COUNT:
        raise ValueError(
            f"Relay channel must be an integer from 0 to {NUMATO_RELAY_COUNT - 1}."
        )
...
def extract_parameters(V, I, area_cm2, pin_mw_cm2=100):
    ...
    if np.isfinite(Jsc_raw) and not np.isclose(Jsc_raw, 0.0):
        ...
        current_polarity = (
            -1 if abs(np.nanmin(measured_power)) > abs(np.nanmax(measured_power)) else 1
        )
    ...
    return {
        ...
        "Rs_derivative": float(Rs_derivative)
        if np.isfinite(Rs_derivative) else np.nan,

        "Rsh_derivative": float(Rsh_derivative)
        if np.isfinite(Rsh_derivative) else np.nan,

        "Rs_diode_eq": float(Rs_diode_eq)
        if np.isfinite(Rs_diode_eq) else np.nan,

        "Rsh_diode_eq": float(Rsh_diode_eq)
        if np.isfinite(Rsh_diode_eq) else np.nan,
    }
...
class GUI(QWidget):
    ...
    def add_result_row(self, pixel, area, metrics, status):
